...
function makeRoughnessMap(strength: number): HTMLCanvasElement {
	const S = 512;
	const cv = document.createElement('canvas');
	cv.width = S;
	cv.height = S;
...
export default function ThreeStage(props: StageProps) {
	...
	const reflectorRef = React.useRef<Reflector | null>(null);
	const reflectorOverlayRef = React.useRef<THREE.Mesh | null>(null);
	const rimLightRef = React.useRef<THREE.DirectionalLight | null>(null);
	...
	const roughnessMapRef = React.useRef<THREE.CanvasTexture | null>(null);
	...
	// Cached load result for the current logo URL. Either the parsed
	// silhouette shapes (raster/SVG path) or the loaded GLB scene
	// wrapped + normalised. Splitting the fetch from the geometry build
	// keeps thickness/bevel slider edits cheap (no refetch).
	type CachedLoad =
	...
	const geomBboxRef = React.useRef<{ minY: number; maxY: number } | null>(null);
	...
	React.useEffect(() => {
		...
		if (cached.kind === 'object') {
			...
			group.add(sceneRoot);
			meshRef.current = sceneRoot as any;
			return;
		}
		...
		const handle = setTimeout(() => {
			...
			const curveSegments = Math.max(
				10,
				Math.round(24 / Math.sqrt(bevelSegments + 1)),
			);
			...
			if (typeof console !== 'undefined') {
				...
				console.log(
					`[extruded-monolith] ExtrudeGeometry build=${(tBuild1 - tBuild0).toFixed(1)}ms ` +
	...
	React.useEffect(() => {
		...
		if (reflectorOverlayRef.current) {
			...
			(reflectorOverlayRef.current.material as THREE.Material).dispose();
			(reflectorOverlayRef.current.geometry as THREE.BufferGeometry).dispose();
	...
	React.useEffect(() => {
		if (keyLightRef.current) keyLightRef.current.color = hexToColor(keyColor);
	}, [keyColor]);
	React.useEffect(() => {
	...
	useHeroAnimationFrame(rootRef, ({ delta, elapsed }) => {
		...
		const layoutX =
			layout === 'content-left' ? 0.95 : layout === 'content-right' ? -0.95 : 0.0;
		...
		group.rotation.set(
			pitchRef.current,
			ambientYawRef.current + yawRef.current,
			0,
		);
		...
		if (rim) {
			...
			const r = 4.2;
			rim.position.set(
				Math.cos(t) * r + layoutX,
				0.8 + 0.35 * Math.sin(t * 0.7),
				Math.sin(t) * r,
			);
