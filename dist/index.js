...
function ExtrudedLogoMonolithHero(props) {
    const { size, input, reducedMotion, rootRef, layout = 'content-left', logoSvg = 'https://crazygl.com/samples/crazygl.svg', monolithSize = 1.05, thickness = 0.32, bevel = 0.6, rotationSpeed = 0.18, parallaxStrength = 0.5, logoColor = '#DeDeDe', metalness = 0.72, roughness = 0.22, imperfectionStrength = 0.35, keyColor = '#ffeecf', rimColor = '#7aa2ff', rimIntensity = 1.0, rimSweepSpeed = 0.35, envIntensity = 1.1, showFloor = true, useGlbMaterials = true, transparent = false, bgTop = '#10131c', bgBottom = '#04050a', } = props;
    ...
    return (_jsxs(_Fragment, { children: [_jsx("crazygl-stage", { style: { position: 'absolute', inset: 0, zIndex: 0, overflow: 'hidden', ...bgStyle }, children: mounted ? (_jsx(React.Suspense, { fallback: null, children: _jsx(ThreeStage, { rootRef: rootRef, size: size, input: input, reducedMotion: reducedMotion, layout: layout, logoSvg: logoSvg, monolithSize: monolithSize, thickness: thickness, bevel: bevel, rotationSpeed: rotationSpeed, parallaxStrength: parallaxStrength, logoColor: logoColor, metalness: metalness, roughness: roughness, imperfectionStrength: imperfectionStrength, keyColor: keyColor, rimColor: rimColor, rimIntensity: rimIntensity, rimSweepSpeed: rimSweepSpeed, envIntensity: envIntensity, showFloor: showFloor, useGlbMaterials: useGlbMaterials, transparent: transparent }) })) : null }), _jsx("crazygl-content", { style: {
                    position: 'absolute',
                    inset: 0,
                    display: 'flex',
                    alignItems: 'center',
                    ...
                    pointerEvents: 'none',
                    ...contentAlign,
                }, children: _jsx("div", { className: "crazygl-elm-content", children: content.node }) })] }));
}
export { metadata };
export default function ExtrudedLogoMonolith(props) {
    return _jsx(CrazyGLWrapper, { hero: ExtrudedLogoMonolithHero, metadata: metadata, ...props });
}
